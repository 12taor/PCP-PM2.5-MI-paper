...
line 86: what year is this population from and why every year gets the same value?
...
119-184: I think it would be really helpful to anotate so that is easier for people to follow up

127: why did you multiply all components by 1000
```{r}
...
238: Just curious, is the 4 df per year for the seasons?
238: I think the figure legend in the supplements should include that this is a smooth line fit with 36 df. 
```{r include=FALSE}
...
380-452: This is nothing to do with the code, but not fully understand why are we using the norm L1 (sum of absolute values in the vector), to estimate variance
```{r}
