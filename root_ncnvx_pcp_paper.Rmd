...
```{r}
# install pcpr
install.packages("/Users/yn2295/Library/Mobile Documents/com~apple~CloudDocs/Columbia Univ./Science/Dr.Marianthi-Anna/Postdoc_research/PCP/PCPr", repos = NULL, type = "source")

install.packages("/Users/yn2295/Library/Mobile Documents/com~apple~CloudDocs/Columbia Univ./Science/Dr.Marianthi-Anna/Postdoc_research/PCP/PCPhelpers", repos = NULL, type = "source")
```
...
154: Maybe add a note on what you mean by corrupting (e.g. replacing 15% of observations with NAs, this 15% of observations becomes the test set).
...
line 174: you estimate the default tuning parameters but then don't use them for anything else, right? maybe add a note next to the code that we are actually not using those values for anything 
164: But multiplying a number by 0 does affect the calculation.. maybe I'm missing something
...
# corrupt data
# Generates a list that inside contains 2 list: $cor.mat (corrupted matrix) and $cor.mask (1=corrupted values, 0=non-corrupted values)
