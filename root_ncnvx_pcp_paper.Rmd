...
source(here::here("law_experiments/nhanes_nyc_tests/eda-latest-nyc-data.R"))
...
pm25_tot <- read_csv("./rachel_experiments/pm2.5_total_components_daily_nyc_avg.csv") %>% 
  rename(
    date = date_local,
    pm25 = pm2.5_total
  ) %>% 
...
# write_csv(nyc_raw, "./rachel_experiments/figures/nyc_raw.csv")
...
# last statement that gets combined with rbind in this example:
data.frame(
...
```{r}
# run NMF using different algorithms to find the one we want to use:
res.multi.method <- nmf(
  ...
  # .parameters = list("pe-nmf" = list("alpha" = 4, "beta" = 4))) # params for some algos that need params
...
# write_csv(nmf_loadings, "./rachel_experiments/figures/pcp_nmf_loadings_offset.csv")
...
# write_csv(nmf_scores, "./rachel_experiments/figures/pcp_nmf_scores_offset.csv")

```

Figure 1: loadings figure
```{r}
pcp_nmf_loadings <- read_csv("./rachel_experiments/figures/pcp_nmf_loadings_offset.csv") %>% 
...
# write_csv(sparse_df, "./root_ncnvx_sparse_mat.csv")
```
